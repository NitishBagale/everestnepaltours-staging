...
xdebug.log={TMPDIR}/{RUNID}issue2069-001.txt
--FILE--
<?php
...
echo "\n", file_get_contents(sys_get_temp_dir() . '/' . getenv('UNIQ_RUN_ID') . 'issue2069-001.txt' );
@unlink (sys_get_temp_dir() . '/' . getenv('UNIQ_RUN_ID') . 'issue2069-001.txt' );
?>
--EXPECTF--
string(%d) "%send"
