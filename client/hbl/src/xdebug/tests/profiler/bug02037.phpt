...
--INI--
xdebug.mode=profile
xdebug.log={TMPDIR}/{RUNID}issue2037.txt
xdebug.output_dir=/tmp/un-writable
--FILE--
<?php
echo "==DONE==\n";
echo file_get_contents(sys_get_temp_dir() . '/' . getenv('UNIQ_RUN_ID') . 'issue2037.txt' );
@unlink (sys_get_temp_dir() . '/' . getenv('UNIQ_RUN_ID') . 'issue2037.txt' );
?>
--EXPECTF--
==DONE==
[%d] Log opened at %s
